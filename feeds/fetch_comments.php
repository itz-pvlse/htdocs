...
// 1. Unified Query - Fetching rich user data and like status
...
// 2. Map comments by ID for tree construction
$commentMap = [];
...
/**
 * 4. Recursive Function - Mobile Friendly + Full Logic
 */
function renderComments($comments, $postId, $current_user_id, $depth = 0, $parentHandle = null) {
    foreach ($comments as $comment) {
        ...
        /**
         * UPDATED: Horizontal Space Management
         * Caps indentation to prevent text from becoming too narrow on mobile.
         */
        $maxIndentDepth = 2;
        $indentClass = "";
        if ($depth > 0) {
            $marginSize = ($depth <= $maxIndentDepth) ? 'ml-6' : 'ml-1';
            $borderColor = ($depth > $maxIndentDepth) ? 'border-blue-500/20' : 'border-[#2D2D2E]';
            $indentClass = "{$marginSize} border-l {$borderColor} pl-4 mt-4";
        } else {
        ...
        <div id="comment-wrapper-<?= $comment['id'] ?>" 
            ...
            <div class="absolute top-4 right-2 z-50">
                <button onclick="toggleCommentMenu(event, <?= $comment['id'] ?>)" 
            ...
            <?php if ($isOwner): ?>
                <div id="delete-overlay-<?= $comment['id'] ?>" 
                     class="hidden absolute inset-0 bg-black/95 backdrop-blur-md flex items-center justify-center z-[60] rounded-xl">
            ...
            <div class="flex gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <?php if ($depth > $maxIndentDepth): ?>
                            <span class="text-[7px] border border-blue-500/30 text-blue-400/60 px-1 rounded uppercase font-black">Deep Thread</span>
                        <?php endif; ?>
                    </div>
